...
const (
	ApiFeePercent = 0.0012 // 手数料
)

type AI struct {
	API                  *bitflyer.APIClient
	ProductCode          string  // ex) BTC_JPY
	CurrencyCode         string  // ex) BTC
	CoinCode             string  // ex) JPY
	UsePercent           float64 // 資産の何％使うか
	MinuteToExpires      int     // オーダーを出してどれくらい保持させるか
	Duration             time.Duration
	PastPeriod           int
	SignalEvents         *models.SignalEvents
	OptimizedTradeParams *models.TradeParams
	TradeSemaphore       *semaphore.Weighted // 取引するときは一つのゴールーチンにするため
	StopLimit            float64
...
func (ai *AI) WaitUntilOrderComplete(childOrderAcceptanceID string, executeTime time.Time) bool {
	...
	expire := time.After(time.Minute + (20 * time.Second)) // 1分20秒見てオーダーが成立したかみる
	interval := time.Tick(15 * time.Second)                // 15秒おきにリストオーダーをみる
